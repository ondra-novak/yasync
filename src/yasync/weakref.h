...
class WeakRef {
	...
	static const unsigned int initedFlag = 1 << (sizeof(unsigned int) * 8 - 1);
	static const unsigned int leftFlag = 1 << (sizeof(unsigned int) * 8 - 2);

	class Pointer: public RefCntObj {
	public:
		...
		AlertFn clearOutNotify;

		Pointer(T *ptr)
			:ptr(ptr)
			,lockCount(initedFlag)
			,clearOutNotify(nullptr) {}
	};
...
protected:
	...
	void setNull() {
			...
			unsigned int v = initedFlag;
			ref->lockCount.compare_exchange_strong(v,leftFlag);
			...
			if (v > initedFlag) {
				...
				AlertFn sleeper = AlertFn::thisThread();
				...
				while (v) {
					...
					v = ref->lockCount.load(std::memory_order_acquire);
...
public:
	...
	WeakRefPtr(const WeakRef<T> &ref)
		:ptr(ref.lockInternal()),me(ref) {}

	WeakRefPtr(const WeakRefPtr &other)
		:ptr(other.me.lockInternal()),me(other.me) {}
...
template<typename T>
inline WeakRefPtr<T> WeakRef<T>::lock() const {
